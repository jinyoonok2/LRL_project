...
def describe_config(config: dict[str, Any], args: argparse.Namespace) -> None:
    ...
    print(f"task_count={len(tasks)} index_count={len(indices)} episode_count={len(tasks) * len(indices)}")
...
def run_eval(config: dict[str, Any], args: argparse.Namespace) -> int:
    ...
    for task in tasks:
        ...
        for idx in indices:
            ...
            output_root = output_base_dir / f"{task}_idx_{idx}_{metadata['slug']}"
            run_info_path = output_root / "run_info.json"
            if not args.dry_run:
            ...
            log_file = log_dir / f"{task}_idx_{idx}.log"
            cmd = [
                sys.executable,
            ...
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Starting task={task} idx={idx}")
